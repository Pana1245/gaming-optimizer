...
const Bar = ({ label, value, color }: { label: string; value: number; color: string }) => (
  <div className="flex items-center gap-2">
    <span className="text-[11px] font-medium text-text-mute w-7">{label}</span>
    ...
    <span className="text-[11px] text-text-dim tabular-nums w-8 text-right">{value.toFixed(0)}%</span>
  </div>
);
  ...
  return (
    <div className="mx-1 mb-1 p-3 rounded-lg bg-white/[0.02] border border-line space-y-2.5">
      <div className="flex items-center justify-between mb-0.5">
        <span className="text-[11px] font-semibold tracking-wider text-text-mute uppercase">{t("mon.performance")}</span>
